...
def build_model(nx: Optional[int] = None,
    ...
    # Reshape inputs to have the correct last dimension
    #reshaped_inputs = tf.keras.layers.Lambda(lambda x: tf.expand_dims(x[:, :, :, 0], axis=-1))(inputs)
    ...
    outputs = layers.Activation("sigmoid", name="outputs")(x)
